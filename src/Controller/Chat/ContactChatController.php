...
/**
 * @Route("/chat", defaults={"_locale"="es"})
 */
class ContactChatController extends Controller {
    ...
    public function createConversation(Request $request, UserRepository $users): JsonResponse {
        ...
        return new JsonResponse([
            ...
            "last_time" => null,
            "unseen" => 0,
            "member" => ['id' => $member->getId(), 'name' => $member->getFullname(), 'avatar' => $member->getAvatarPath()]
                ], 200);
    }
     ...
     * @Method("GET")
     */
    public function getConversationMessages(Request $request, ContactConversationRepository $conversationRepo, $id): JsonResponse {
        $this->denyAccessUnlessGranted('IS_AUTHENTICATED_FULLY');
        $user_id = $this->getUser()->getId();
        Carbon::setLocale($request->getLocale());
     ...
     * @Method("POST")
     */
    public function refreshConversationMessages(Request $request, ContactConversationRepository $conversationRepo, $id): JsonResponse {
        $this->denyAccessUnlessGranted('IS_AUTHENTICATED_FULLY');
        $user_id = $this->getUser()->getId();
        Carbon::setLocale($request->getLocale());
        $conversation = $conversationRepo->find($id);
     ...
     * @Method("POST")
     */
    public function sendMessage(Request $request, ContactConversationRepository $conversationRepo, $id): JsonResponse {
        $this->denyAccessUnlessGranted('IS_AUTHENTICATED_FULLY');
        $user_id = $this->getUser()->getId();
        $conversation = $conversationRepo->find($id);
        if (!$conversation || !$conversation->inConversation($user_id)) {
